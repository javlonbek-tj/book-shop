  ...
  if (
    ...
    if (
      ...
      if (showError) {
        ...
        errorSpan.textContent = `The ${
          input.id
        } should have no spaces and at least ${
          input.id === "name" ? 4 : 5
        } characters.`;
      }
    } else {
...
form.addEventListener("submit", (e) => {
  ...
  const payment = document.querySelector('input[name="payment"]:checked')
    ? document.querySelector('input[name="payment"]:checked').nextElementSibling
        .textContent
    : "None";

  const selectedGifts = [];
  document
    .querySelectorAll('.checkout__gift-options input[type="checkbox"]')
    .forEach((checkbox) => {
      if (checkbox.checked) {
  ...
  const orderSummary = `
    ...
    <p><strong>Gifts:</strong> ${
      selectedGifts.length > 0 ? selectedGifts.join(", ") : "No gifts"
    }</p>
    </div>
